        ...
        for i in range(L):
            for j in range(aln.shape[0]):
              try:
                ...
                hhblits_profile[i, mapping['X']] += 1
                reweighted_profile[i, mapping['X']] += seq_weight[j]
        ...
        for i in range(L):
            for j in aln[:, i]:
              try:
                ...
                non_gapped_profile[i, mapping['X']] += 1
        non_gapped_profile[:, -1] = 0
        non_gapped_profile /= non_gapped_profile.sum(-1).reshape(-1, 1)

        mapping = {aa: i for i, aa in enumerate('-ARNDCQEGHILKMFPSTWYVX')}
        a2n = np.frompyfunc(lambda x: mapping['X'] if (x == 'B' or x=='Z') else mapping[x], 1, 1)
